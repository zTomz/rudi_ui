...
- `RudiFloatingNavigationBar`: compact floating navigation with a continuously moving selection indicator, Loop-aligned light/dark defaults, selected/unselected color slots, RTL, keyboard focus and reduced motion support. Supply `selectedIcon` on each destination for a filled variant.
...
- `RudiSettingsGroup` and `RudiSwitchTile`: separated setting rows with grouped outer corners and a single accessible toggle target. Selected `RudiSettingsTile` rows use a neutral foreground-colored background with contrasting labels, subtitles and inherited icon colors.
